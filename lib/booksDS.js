...
let dbo = null;
    ...
    const findOne = async(code) => {
        try {
            const result = await dbo.collection(keys.collectionName).findOne({code: String(code)})             
            return result            
        } catch (err) {
            throw err
        }
    ...
    const UpdateBook = async(book) => {
        try {
            ...
            const { code, ...updateFields } = book; // Exclude 'code' from update
            var newvalues = { $set: updateFields };
